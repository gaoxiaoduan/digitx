...
function structureReason(number: string): QualityReason | null {
...
function matchStrictAnchor(number: string, anchors: readonly string[]): AnchorMatch | null {
  for (const anchor of [...anchors].sort((left, right) => right.length - left.length)) {
    ...
    const minimumPadding = anchor.length >= 5 ? 1 : anchor.length === 4 ? 2 : 3;
    const placements = [
...
function geekReason(number: string): QualityReason | null {
  const match = matchStrictAnchor(number, GEEK_ANCHORS);
...
function publicMemoryReason(number: string): QualityReason | null {
  const match = matchStrictAnchor(number, PUBLIC_MEMORY_ANCHORS);
  if (!match) return null;
  return {
...
function luckyReason(number: string): QualityReason | null {
  ...
  if (number === '5201314' || number === '1314520') {
    return {
  ...
  for (const anchor of ['168', '518', '528', '520', '521', '1314', '3344']) {
    if (number.length % anchor.length === 0 && anchor.repeat(number.length / anchor.length) === number) {
      return {
...
function dateReason(number: string): QualityReason | null {
...
function cityReason(number: string): QualityReason | null {
  for (const city of CITY_AREA_CODES) {
    ...
    return {
      ...
      tags: ['城市区号', city.city, isPureRepeat(suffix) ? '重复后缀' : '顺子后缀']
    };
  }
  return null;
}
...
function addStructuralSeeds(seeds: Set<string>): void {
  const digits = [...ASCENDING_DIGITS];
  for (const length of [6, 7, 8]) {
    for (const digit of digits) seeds.add(digit.repeat(length));

    if (length === 6) {
      for (const dominant of digits) {
        for (const outlier of ['0', '8']) {
          if (dominant === outlier) continue;
          seeds.add(dominant.repeat(length - 1) + outlier);
          seeds.add(outlier + dominant.repeat(length - 1));
        }
      }
    }
  }
  ...
  for (const first of digits) {
    for (const second of digits) {
      ...
      seeds.add(first.repeat(3) + second.repeat(3));
      seeds.add((first + second).repeat(3));
    }
  }
  ...
  for (const sequence of [ASCENDING_DIGITS, DESCENDING_DIGITS]) {
    for (const length of [6, 7, 8]) {
      for (let start = 0; start <= sequence.length - length; start += 1) {
        seeds.add(sequence.slice(start, start + length));
      }
    }
  }
...
function addAnchorSeeds(seeds: Set<string>, anchors: readonly string[]): void {
  for (const anchor of anchors) {
    if (anchor.length >= 6 && anchor.length <= 8) seeds.add(anchor);
    for (const length of [6, 7, 8]) {
      if (length % anchor.length === 0) seeds.add(anchor.repeat(length / anchor.length));
      const paddingLength = length - anchor.length;
      const minimumPadding = anchor.length >= 5 ? 1 : anchor.length === 4 ? 2 : 3;
      if (paddingLength < minimumPadding) continue;
      for (const digit of ['0', '6', '8', '9']) {
        seeds.add(anchor + digit.repeat(paddingLength));
      }
    }
  }
}

function addSemanticSeeds(seeds: Set<string>): void {
  addAnchorSeeds(seeds, GEEK_ANCHORS);
  addAnchorSeeds(seeds, PUBLIC_MEMORY_ANCHORS);
  ...
  for (const anchor of LUCKY_ANCHORS) {
    for (const length of [6, 7, 8]) {
      if (length % anchor.length === 0) seeds.add(anchor.repeat(length / anchor.length));
      const paddingLength = length - anchor.length;
      const minimumPadding = anchor.length >= 4 ? 2 : 3;
      if (paddingLength < minimumPadding) continue;
      for (const digit of ['6', '8']) {
        seeds.add(anchor + digit.repeat(paddingLength));
      }
    }
  }
  ...
  for (const city of CITY_AREA_CODES) {
    for (const length of [6, 7, 8]) {
      ...
      for (const digit of ['0', '4', '6', '8', '9']) {
        seeds.add(city.code + digit.repeat(suffixLength));
      }
    }
  }
...
export function generateCandidates(options: GeneratorOptions = {}): GeneratedCandidate[] {
  ...
  addStructuralSeeds(seeds);
  addSemanticSeeds(seeds);
