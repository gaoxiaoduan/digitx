import { EvaluationResult, GeneratorOptions, CityInfo, DomainRecord } from './types.js';

export const TECH_CONSTANTS: string[] = ['1024', '2048', '4096', '8192', '404', '500', '502', '127001', '12306', '996', '955'];
export const LUCKY_SEGMENTS: string[] = ['168', '520', '521', '1314', '666', '888', '999', '518', '528', '3344'];
...
export function hasUnlucky4(str: string): boolean {
  if (!str.includes('4')) return false;

  const len = str.length;
  const isPartOfAllowed = new Array<boolean>(len).fill(false);
...
export function evaluateNumber(numStr: string, exclude4: boolean = true): EvaluationResult {
  ...
  if (len === 8) {
    ...
    if (year >= 1980 && year <= 2025 && month >= 1 && month <= 12 && day >= 1 && day <= 31) {
      ...
      const isRepDate = (numStr.slice(4, 6) === numStr.slice(6, 8));
  ...
  for (const cityInfo of CITY_AREA_CODES) {
    if (numStr.startsWith(cityInfo.code)) {
      ...
      const isLeopard = new Set(suffix).size === 1;
  ...
  if (contains1024 && contains404) {
    return { score: 96, category: '极客神号', patternDesc: '程序员双击: 1024 (程序员节) + 404 (找不到页面)' };
  }
  if (contains127001) {
    return { score: 95, category: '极客神号', patternDesc: '本地环回主机 IP (127.0.0.1)' };
  }
  if (contains1024 && len === 6) {
    const remaining = numStr.replace('1024', '');
    const isLucky = ['88', '66', '99', '16', '68'].includes(remaining);
    return { score: isLucky ? 94 : 90, category: '程序员优选', patternDesc: `极客 1024 携带精选尾数 (${remaining})` };
  }
  if (contains12306) {
    const remaining = numStr.replace('12306', '');
    const isLucky = ['2', '8', '6', '9', '1'].includes(remaining);
    return { score: isLucky ? 93 : 89, category: '程序员优选', patternDesc: `中国铁路客服 12306 经典组合 (${numStr})` };
  }

  // 4. CHINESE LUCKY PUNS COMBINATIONS
  if ((numStr.includes('520') || numStr.includes('521')) && numStr.includes('1314')) {
    return { score: 95, category: '国人吉祥号', patternDesc: '浪漫爱情誓言: 一生一世我爱你 (5201314 / 1314520)' };
  }
  if (numStr.startsWith('168') && (numStr.endsWith('888') || numStr.endsWith('8888') || numStr.endsWith('88888'))) {
    return { score: 94, category: '国人吉祥号', patternDesc: '一路发发发: 168 (一路发) + 连号 8s (发财)' };
  }
  if (numStr.startsWith('518') && (numStr.endsWith('888') || numStr.endsWith('168'))) {
    return { score: 91, category: '国人吉祥号', patternDesc: '我要发发发: 518 (我要发) + 财富组合' };
  }
  ...
  if (uniqueDigits.size === 2) {
    ...
    if (freq[0] === len - 1) {
      ...
      return { score: isPremiumDominant ? 93 : 88, category: '极品豹子', patternDesc: `多重重复连号 ${dominantDigit} (多合一极品)` };
    }

    if (len === 8) {
      if (freq[0] === 4 && freq[1] === 4) {
        if (numStr.slice(0, 4) === numStr[0].repeat(4) && numStr.slice(4) === numStr[4].repeat(4)) {
          return { score: 91, category: '双重连号', patternDesc: '半对半分立双连号 (AAAABBBB)' };
        }
        if (numStr.slice(0, 2) === numStr.slice(4, 6) && numStr.slice(2, 4) === numStr.slice(6, 8)) {
          return { score: 88, category: '双重连号', patternDesc: '节奏双重对子重复 (AABBAABB)' };
        }
      }
    }
    ...
    if (len === 6 && freq[0] === 3 && freq[1] === 3) {
      if (numStr.slice(0, 3) === numStr[0].repeat(3) && numStr.slice(3) === numStr[3].repeat(3)) {
        return { score: 90, category: '双重连号', patternDesc: '三叠双连号豹子 (AAABBB)' };
      }
    }
  }

  // 6. TRIPLE REPEAT PAIRS
  if (len === 6) {
    if (numStr[0] === numStr[1] && numStr[2] === numStr[3] && numStr[4] === numStr[5]) {
      return { score: 87, category: '对子连号', patternDesc: '三对子叠号步步高 (AABBCC)' };
    }
  }
  if (len === 8) {
    if (numStr[0] === numStr[1] && numStr[2] === numStr[3] && numStr[4] === numStr[5] && numStr[6] === numStr[7]) {
      return { score: 88, category: '对子连号', patternDesc: '四对子华丽叠号 (AABBCCDD)' };
    }
  }
  ...
  if (len === 6) {
    ...
    if (numStr === abab.repeat(3)) {
      return { score: 89, category: '交替循环号', patternDesc: '双数字节奏交替循环 (ABABAB)' };
    }
    const abc = numStr.slice(0, 3);
    if (numStr === abc.repeat(2)) {
      const scoreVal = abc === '168' ? 93 : 88;
      return { score: scoreVal, category: '交替循环号', patternDesc: `三数字周期叠字循环 (${abc}${abc})` };
    }
  }
  if (len === 8) {
    const abab = numStr.slice(0, 2);
    if (numStr === abab.repeat(4)) {
      return { score: 89, category: '交替循环号', patternDesc: '双数字长字节交替循环 (ABABABAB)' };
    }
    const abcd = numStr.slice(0, 4);
    if (numStr === abcd.repeat(2)) {
      return { score: 87, category: '交替循环号', patternDesc: '四数字大型交替循环 (ABCDABCD)' };
    }
  }
  ...
  if (isPalindrome) {
    return { score: len >= 7 ? 88 : 85, category: '对称镜像号', patternDesc: `左右完全对称回文镜像号 (${numStr})` };
  }

  // 9. GENERAL LUCKY SCORE
  let luckyCount = 0;
  ...
  if (luckyCount >= 5) {
    return { score: Math.min(85, 60 + luckyCount * 4), category: '国人吉祥号', patternDesc: '高浓度传统大吉数字搭配' };
  }

  return { score, category, patternDesc };
}

export function generateCandidates(options: GeneratorOptions = {}): Omit<DomainRecord, 'status' | 'detail' | 'updatedAt'>[] {
  const {
    minLength = 6,
    maxLength = 8,
  ...
  const addCandidate = (numStr: string | number) => {
  ...
  for (const len of [6, 7, 8]) {
      ...
      for (const d1 of digits) {
        for (const d2 of digits) {
          if (d1 !== d2) {
            ...
            addCandidate(d1.repeat(2) + d2.repeat(2) + d1.repeat(2) + d2.repeat(2));
          }
        }
      }
    }
    ...
    for (const d1 of digits) {
      for (const d2 of digits) {
        if (d1 !== d2) {
          const pair = d1 + d2;
          if (len === 6) addCandidate(pair.repeat(3));
          if (len === 8) addCandidate(pair.repeat(4));
        }
      }
    }
  ...
  const straights = ['0123456789', '9876543210'];
  ...
  // GENERATOR WORKFLOW 6: DATES & ANNIVERSARIES
